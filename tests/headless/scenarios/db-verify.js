...
scenarios.push({
  id: 105,
  regression: true,
  name: "DB live: HardDelete re-parents, clears FTS, leaves cumulative (bug #65)",
  mode: null,
  settings:{},
  fixtures:{
  ...
  async body({cdp, dbPath}){
    ...
    if(Number(thread.cumulative_cost)!==1.5) throw new Error("cumulative changed "+JSON.stringify(thread));
    ...
    return "HardDelete OK: reparent, FTS cleared, cumulative stale 1.5, active_path 20";
  }
});
scenarios.push({
  id: 106,
  regression: true,
  name: "DB live: Fork copies messages, drops folder/cumulative",
  mode: null,
  settings:{},
  fixtures:{
  ...
  async body({cdp, dbPath}){
    ...
    if(fork.folder_id==="f106") throw new Error("folder kept");
    if(Number(fork.cumulative_cost)!==0) throw new Error("cumulative "+JSON.stringify(fork));
    return "fork "+nid+" msgs "+cnt+" folder dropped cumulative 0";
  }
});
