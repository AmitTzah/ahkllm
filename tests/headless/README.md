...
| `launch.js` | Profile isolation (junction), app launch with WebView2 remote-debugging args, CDP discovery, teardown |
  ...
  | `probe-bughunt-db.ahk` | Standalone AHK check that runs the REAL ChatDB/repo code against a temp SQLite DB and prints token-accounting / tree-copy / cost-snapshot / provider-resolution results (used by the DB-audit scenarios; see report). `open-race <dbPath>` opens a shared DB so two spawned processes can race the startup schema/FTS-rebuild path (scenario 185) |
