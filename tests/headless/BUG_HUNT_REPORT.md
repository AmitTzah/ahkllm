...
- **Where we left off:** 2026-08-14 - Bug hunt round 6 COMPLETE: #228 verified,
  fixed + committed (1252ba6) and closed in History. Final verification green:
  full headless suite 222/222 PASS and `npm run test:fast` (607 AHK + 571 JS).
...
- 2026-08-14 - "Commands with an empty API Model (the "Default" dropdown
  ...
  dropped the empty-valued properties, so cmd.APIModels / cmd.commandName /
  cmd.menuText threw in AHK v2 before the #162 default-model substitution" -
  FIXED + COMMITTED in 1252ba6: fix(settings): SettingsApply._ApplyCommands
  now assigns command keys whenever the saved key exists (empty included, the
  #101/#61/#71 pattern) via _SetIfExists, and CommandMenu/CommandState guard
  every direct read. Scenario 228 flipped to a regression check + new
  SettingsHandler AHK unit test; full headless suite 222/222 PASS and
  test:fast green (607 AHK + 571 JS).

- 2026-08-13 - "Settings saves (and all Main-bound IPC) are silently dropped when another AHK script's window shadows Main's own" - FIXED + COMMITTED in 92e6ade: fix(main): Main.ahk now resolves its own window with A_ScriptHwnd in the prewarm spawn and _spawnChatWindow (was the ambiguous WinExist("ahk_class AutoHotkey") class lookup, which could return one of the user's other AHK scripts' windows). Scenario 227 flipped to a regression check + new MainWindowHandle AHK unit tests; full headless suite 221/221 PASS and test:fast green (606 AHK + 571 JS).
- 2026-08-13 - "Sidebar New Chat discards right-rail selections made before the first message" - REFUTED: clicking New Chat is explicitly "start fresh with the configured defaults" (user-confirmed expected behavior), so pre-send selections are reset by design. Scenario 226 kept as a regression check (regression: true) guarding the reset behavior.
- 2026-08-13 - "Sidebar New Chat drops the pre-send font-size adjustment" - REFUTED: a new chat intentionally starts at the configured default font (user-confirmed expected behavior). Scenario 225 kept as a regression check (regression: true) guarding the reset behavior.
...
- 2026-08-12 - "Fresh-profile default assistant loses isDefault (defaults snapshot drops the flag)" - FIXED + COMMITTED in c71bc0a: fix(settings): defaults snapshot preserves assistant isDefault for fresh profiles (bug #196). Scenario 196 flipped to a regression check + unit tests.
- 2026-08-12 - "Switching branches within the same thread mid-stream mis-parents the completed response" - FIXED + COMMITTED in 368a720: fix(stream): capture request parent at send time so branch switches cannot re-parent responses (bug #197). Scenario 197 flipped to a regression check + unit tests.
- 2026-08-12 - "PDF/office attachments with a generic MIME type are misclassified as text_file" - FIXED + COMMITTED in 6e6cf37: fix(chat): classify generic-MIME PDF/office attachments by extension (bug #198). Scenario 198 flipped to a regression check + unit tests.
- 2026-08-12 - "Zero configured providers crashes the API-key error handler" - FIXED + COMMITTED in b9894cd: fix(api): provider-less API-key error handler no longer crashes (bug #199). Scenario 199 flipped to a regression check + unit tests.
- 2026-08-12 - "Command usage rows with an empty provider and a provider-prefixed model are unfilterable" - FIXED + COMMITTED in aeadb0f: fix(dashboard): command rows chart under the model provider prefix, not an unfilterable empty key (bug #200). Scenario 200 flipped to a regression check + unit tests.
- 2026-08-12 - "Assistant Reasoning dropdown drops short-form base model metadata" - FIXED + COMMITTED in 92a2503: fix(settings): assistant reasoning dropdown resolves short-form model ids (bug #201). Scenario 201 flipped to a regression check + unit tests.
- 2026-08-12 - "Forking a thread with a "Save as Branch" local copy re-counts the copy as a real API call" - FIXED + COMMITTED in 71eebb4: fix(chat): forks preserve is_local_copy so branch copies are never re-charged (bug #202). Scenario 202 flipped to a regression check + unit tests.
- 2026-08-12 - "Chat-mode command "Stream Response" OFF is ignored" - FIXED + COMMITTED in c8ac8d8: fix(commands): chat-mode commands honor Stream Response OFF via single-shot JSON path (bug #203). Scenario 203 flipped to a regression check + unit tests.
- 2026-08-12 - "A stalled stream never times out (no --max-time on the streaming cURL command)" - FIXED + COMMITTED in 10e4498: fix(api): streaming requests carry an overall --max-time so stalled streams cannot hang forever (bug #204). Scenario 204 flipped to a regression check + unit tests.
- 2026-08-12 - "Cancelling a retry of a ROOT assistant inserts the partial as a child of the original" - FIXED + COMMITTED in eb80a20: fix(stream): cancelled root retry inserts as a sibling, not a child (bug #205). Scenario 205 flipped to a regression check + unit tests.
- 2026-08-12 - "Switching threads mid-stream writes the old request's API-log entry with the new thread's model/provider" - FIXED + COMMITTED in 39098ca: fix(stream): API-log entries use send-time model/provider after thread switches (bug #206). Scenario 206 flipped to a regression check + unit tests.
- 2026-08-12 - "Thread A's stream completion repaints thread B's header token bar with A's stats" - FIXED + COMMITTED in fd7c815: fix(chat): scope token-bar stats to the active thread (bug #207). Scenario 207 flipped to a regression check + unit tests.
