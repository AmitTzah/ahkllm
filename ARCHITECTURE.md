...
│
├── api/                             # API client
│   └── LLMClient.ahk                # LLMClient class: JSON request building, cURL, FIM, response extraction, SSE parsing, logging, cost computation
│
├── ui/                              # UI components
...
2. `Config.ahk` loads all vendor libs + application classes (`LLMClient.ahk`, `InputWindow.ahk`, `CustomMessages.ahk`, `ChatDB.ahk`)
3. `Main.ahk` calls `ChatDB.Open()` to initialize the SQLite database
4. `Main.ahk` creates `router := LLMClient(APIKey)` and three `InputWindow` instances
5. `Main.ahk` registers hotkeys (default: `` ` `` to open menu)
6. `Main.ahk` loads application modules (`app/*.ahk`)
7. Script is ready — tray icon appears, hotkeys active
...
| `title` | TEXT | User-visible thread title |
...
- `undeleteMessage` — undo soft-delete
