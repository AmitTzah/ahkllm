...
class SearchToolExecutor {
    ...
    static Execute(toolCalls, providerInfo, testRunner := "", onSearchProgress := "", loopState := "") {
        ...
        for idx, tc in toolCalls {
            ...
            if name = "web_search" {
                ...
                if query = "" {
                    ...
                    resultText := DeepSeekSearch.Run(query, providerInfo, onSearchProgress, loopState)
                } else {
                    resultText := TavilySearch.Run(query, 5, loopState)
                }
    ...
    static QueueFollowUp(execResult, threadId, parentId, loopCount, loopState := "") {
        if !IsObject(loopState)
            loopState := SearchToolExecutor.NewLoopState(threadId, parentId, Map(), loopCount)
        ctxId := ""
        if loopState.placeholderId != "" {
            ...
            ctxId := ChatDB.Msg_Insert({
                ...
                sibling_group: "",
                sibling_index: 0,
                update_active_leaf: SearchToolExecutor._ShouldUpdateActiveLeaf(threadId, parentId)
            })
            ...
            if ctxId != "" && !SearchToolExecutor._Contains(loopState.contextIds, ctxId) {
                loopState.contextIds.Push(ctxId)
            }
        }
        loopState.toolMessages := execResult.toolMessages
        loopState.loopCount := loopCount
        loopState.params["_pendingToolMessages"] := execResult.toolMessages
        loopState.params["_pendingSearchContextIds"] := loopState.contextIds
        loopState.params["_toolLoopCount"] := loopCount
        if ctxId != "" {
            loopState.parentId := ctxId
            loopState.requestPath := ChatDB.Msg_GetPathToLeaf(threadId, ctxId)
            loopState.params["_requestPath"] := loopState.requestPath
        }
        return ctxId
    }
    ...
    static PrepareFollowUp(toolCalls, threadId, parentId, loopState := "") {
        if !IsObject(loopState)
            loopState := SearchToolExecutor.NewLoopState(threadId, parentId, Map(), 0)
        ctxId := ""
        ...
        if threadId && parentId && query != "" {
            ...
            if ctxId != "" {
                if !SearchToolExecutor._Contains(loopState.contextIds, ctxId)
                    loopState.contextIds.Push(ctxId)
                loopState.placeholderId := ctxId
                loopState.placeholderQuery := query
            }
        }
        return ctxId
    }

    ; Create the per-request continuation state. `params` is a snapshot of
    ; the originating request settings; it is never the visible thread's
    ; requestParams map.
    static NewLoopState(threadId, parentId, params, loopCount := 0) {
        priorContextIds := []
        if IsObject(params) && params.Has("_pendingSearchContextIds") {
            for id in params["_pendingSearchContextIds"] {
                if !SearchToolExecutor._Contains(priorContextIds, id)
                    priorContextIds.Push(id)
            }
        }
        if IsObject(params) && params.Has("_toolLoopCount")
            loopCount := Integer(params["_toolLoopCount"])
        return {
            key: A_TickCount "_" Random(1000, 999999),
            threadId: threadId,
            parentId: parentId,
            params: params,
            requestPath: params.Has("_requestPath") ? params["_requestPath"].Clone() : [],
            toolMessages: [],
            contextIds: priorContextIds,
            placeholderId: "",
            placeholderQuery: "",
            searchPid: 0,
            cancelled: false,
    ...
    static _ShouldUpdateActiveLeaf(threadId, parentId) {
        global activeThreadId
        if activeThreadId != threadId
            return true
        visiblePath := ChatDB.Msg_GetActivePath(threadId)
        return !visiblePath.Length || visiblePath[visiblePath.Length].id = parentId
    }
